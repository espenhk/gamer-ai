...
| `move_exploration_bonus` | `0.01` | Bonus for moving friendly units into a not-yet-visited cell of an 8×8 screen grid. Tracks **actual unit centroid positions** (not move-command targets) and fires at most once per grid cell per episode, so spamming `Move_screen` without relocating units earns nothing (issue #253). |
| `move_repeat_penalty` | `-0.02` | Penalty when a `Move_screen` command is less than `_MOVE_MIN_MEANINGFUL_FRAC` (6/64 ≈ 9% of screen) from the previous move target (covers both exact repeats and tiny stutter steps). |
