...
| `new_action_unlock_bonus` | 5.0 | One-shot bonus the first time a tech-gated fn_idx becomes fully executable in an episode. An action is tech-gated if its preconditions include a required building (e.g. `Train_Marauder_quick` needs BarracksTechLab) **or** its selection target is a production building that itself requires construction (e.g. `Train_Marine_quick` needs Barracks; `Train_Hellion_quick` needs Factory; `Train_Zealot_quick` needs Gateway). Basic-worker trains (`Train_SCV_quick`, `Train_Drone_quick`, `Train_Probe_quick`) and always-available actions (`no_op`, `select_army`, `Move_screen`, `Attack_screen`) are excluded. Recommended range: `1.0–10.0`. |
| `new_action_usage_bonus` | 0.1 | Per-step bonus when the agent actually issues a tech-gated fn_idx that has already been unlocked this episode. Same fn_idx filter as `new_action_unlock_bonus`. Complements it by rewarding sustained use, not just first discovery. Fires up to `new_action_usage_max_uses` times per fn_idx per episode then goes silent. Recommended range: `0.1–2.0` (keep smaller than `new_action_unlock_bonus`). |
...
| `build_bonus` | 0.5 | Per-step bonus when the agent issues a build (structure) category action — supply depots, barracks, factories, refineries, and all other construction fn_idxs (issue #416). Encourages investing mineral income into infrastructure. Fires once per env step; scaled by `n_ticks`. Recommended range: `0.2–1.0`. Set `train_bonus` to `4×` this value. |
| `build_repeat_penalty` | −0.5 | Per-step penalty when the agent issues the same build fn_idx on two consecutive env steps. The first issuance is always unpenalised; any subsequent repetition of the *same* fn_idx without an intervening non-build action fires the penalty. Catches "Can't find placement location" spam loops. Set to `−build_bonus` to make repeated build spam net-zero. Recommended range: `−0.3` to `−1.0`. The shipped ladder config uses `−0.5`. |
| `train_bonus` | 2.0 | Per-step bonus when the agent issues a train (unit production) category action — workers, marines, zealots, and all other unit-production fn_idxs. Intended to be `4 × build_bonus` to reflect that unit production has more immediate combat impact than placing a structure. Fires once per env step; scaled by `n_ticks`. Default: `2.0` (= `4 × 0.5`). |
| `train_repeat_penalty` | −2.0 | Per-step penalty when the agent issues the same train fn_idx on two consecutive env steps. Mirrors `build_repeat_penalty` — catches spam loops where the agent re-queues the same unit without waiting for production to complete. Set to `−train_bonus` to make repeated train spam net-zero. Recommended range: `−0.8` to `−4.0`. The shipped ladder config uses `−2.0`. |
