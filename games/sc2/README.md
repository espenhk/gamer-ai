...
| `build_train_bonus` | 0.5 | Per-step bonus when the agent issues a build or train category action — supply depots, barracks, factories, workers, marines, and all other construction/production fn_idxs (issue #416). Encourages investing mineral income into production rather than cycling through move or attack commands. Fires once per env step; internally scaled by `n_ticks` (the `step_mul` value `SC2Env` passes to `compute_with_components()`), consistent with all other per-step shaping terms. Recommended range: `0.2–1.0` (keep smaller than `supply_growth_bonus` / `worker_growth_bonus` so action-shaping does not dominate outcome-driven signals). Set to `0.0` to disable. |
| `build_repeat_penalty` | 0.0 | Per-step penalty when the agent issues the same build fn_idx on two consecutive env steps. The first issuance of any build fn_idx is always unpenalised; any subsequent repetition of the *same* fn_idx without an intervening non-build action (move, train, no-op, select) fires the penalty. Catches "Can't find placement location" spam loops where a failed placement earns no consequence from the reward function. Set to `−build_train_bonus` (e.g. `−0.5`) to make repeated build spam net-zero reward, breaking the positive feedback loop while leaving the first issuance fully rewarded. Recommended range: `−0.3` to `−1.0`. Opt-in; default `0.0`. The shipped ladder config uses `−0.5`. |
