...
PRECONDITIONS: dict[int, Preconditions] = {
    ...
    # fn_idx 12/13 stay in PRECONDITIONS for table completeness but are excluded
    # from _UNIVERSAL_FN_IDS so they never appear in the race candidate set.
    12: _any(),  # Patrol_screen  (unreachable — not in _UNIVERSAL_FN_IDS)
    13: _any(),  # Patrol_minimap (unreachable — not in _UNIVERSAL_FN_IDS)
...
# ---------------------------------------------------------------------------
# Only fn_idx with a non-zero cost are listed; missing keys → (0, 0).
# Source: Liquipedia LotV unit/building pages.
...
RESOURCE_COSTS: dict[int, tuple[int, int]] = {
    ...
    57: (200, 100),  # Build_RoboticsFacility_screen
    ...
    62: (200, 200),  # Build_RoboticsBay_screen
    63: (300, 200),  # Build_FleetBeacon_screen
    64: (100, 100),  # Build_ShieldBattery_screen
    ...
    78: (300, 200),  # Train_Tempest_quick
    ...
    95: (150, 150),  # Build_NydusNetwork_screen
    96: (150, 150),  # Build_LurkerDen_screen
    97: (50, 0),    # Train_Drone_quick
    98: (100, 0),   # Train_Overlord_quick
    99: (50, 0),    # Train_Zergling_quick
    100: (25, 25),  # Train_Baneling_quick (Morph_Baneling additional cost)
    101: (75, 25),  # Train_Roach_quick
    102: (25, 75),  # Train_Ravager_quick (Morph_Ravager additional cost)
    ...
    108: (150, 150),  # Train_BroodLord_quick (Corruptor morph additional cost)
    109: (100, 200),  # Train_Viper_quick
    110: (300, 200),  # Train_Ultralisk_quick
    111: (50, 100),  # Train_Lurker_quick (Hydralisk morph additional cost)
    112: (150, 0),  # Train_Queen_quick
    113: (150, 100),  # Morph_Lair
    114: (200, 150),  # Morph_Hive
    115: (50, 100),  # Morph_Overseer
    116: (100, 150),  # Morph_GreaterSpire
...
def fn_idx_satisfied(
    ...
    selected_unit_types :
        ...
        selection.
    minerals :
        Current mineral count.  ``float("inf")`` means unconstrained
        (used in unit tests and when player data is unavailable).
    vespene :
        Current vespene count.  Same convention as *minerals*.
    """
    ...
    # Resource check: current balance must cover the action's cost.
