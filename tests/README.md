...
- `TestRaceGating` (9 tests): all four race keys exist in RACE_FUNCTION_IDS; each race's ids are a subset of FUNCTION_IDS; random race = all fn_ids except disabled ones (Patrol_screen/minimap, issue #356); race-specific sets (_TERRAN / _PROTOSS / _ZERG) are pairwise disjoint; unknown race falls back to all non-disabled fn_ids; Terran has Build_Barracks_screen (8) not Build_Nexus_screen (50); Protoss has Build_Nexus_screen (50) not Build_Barracks_screen (8); Zerg has Build_Hatchery_screen (82) not Build_Barracks_screen (8); all three named races include Move_screen (2) and no_op (0)
...
- `TestResourceCostGating` (issue #357): Build_Barracks blocked with 0 or 149 minerals / allowed at exactly 150 or above; Factory blocked with sufficient minerals but insufficient vespene / allowed when both met; no_op and Move_screen always allowed (no cost entry); default `float("inf")` budget is unconstrained; RESOURCE_COSTS only references fn_idx values present in FUNCTION_IDS; fn_idx 12/13 (Patrol) absent from all race sets (issue #356)
...
- resource cost gating (#357): `_compute_available_fn_ids` passes minerals/vespene from the PySC2 player observation to `fn_idx_satisfied`; the tech-tree filter skips resource gating when `_unit_type_id_to_name` is empty (unit-test path)
