...
- new_action_usage_bonus (issue #400): defaults `0.0` / `max_uses=50`; component always zero when disabled; fires for a tech-gated action already unlocked this episode; does not fire before unlock; works independently of `new_action_unlock_bonus`; capped at `new_action_usage_max_uses` uses per fn_idx; non-tech-gated actions (Move_screen) never fire; usage counts reset after `reset()`
