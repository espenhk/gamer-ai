...
- build_train_bonus (issue #416): disabled by default; fires on build fn_idx (8, Build_Barracks_screen); fires on train fn_idx (7, Train_Marine_quick); no bonus on move fn_idx (2) or no_op (0); zero when bonus is 0.0; scales with bonus value; n_ticks scaling; present in components dict even when disabled; components sum equals total
- build_repeat_penalty: disabled by default; first build issuance is never penalised; second consecutive same-fn_idx build fires penalty; third consecutive also fires; different build fn_idx does not fire; intervening move action resets counter; intervening train action resets counter; zero when weight is 0.0; n_ticks scaling; present in components dict when disabled; net zero when penalty equals -build_train_bonus; components sum equals total; reset() clears last build fn_idx
