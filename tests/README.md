...
- build_train_bonus (issue #416): disabled by default; fires on build fn_idx (8, Build_Barracks_screen); fires on train fn_idx (7, Train_SCV_quick); no bonus on move fn_idx (2) or no_op (0); zero when bonus is 0.0; scales with bonus value; n_ticks scaling; present in components dict even when disabled; components sum equals total
