...
- `_FU` column constants (issue #477): `TestFUColumns` verifies all 28 PySC2 `FeatureUnit` columns are present with correct indices, values are unique, and coverage is contiguous (0–27). `TestWorkerOrderLengthPreference` verifies that `_update_unit_screen_positions` caches the worker with the lowest `order_length` (idle beats mining, mining beats building); falls back to first-seen-wins when the array has fewer than 27 columns; non-worker friendly units keep first-seen-wins; and `_pick_typed_selector` returns `select_point` on the lowest-order-length worker when `select_idle_worker` is unavailable.
