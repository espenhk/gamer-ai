...
_BELIEF_CFG = load_belief_config("games/sc2/config/belief_config.yaml")
_BELIEF_EXTRA_DIMS = len(belief_obs_dims(_BELIEF_CFG))
_N_SLOTS = int(_BELIEF_CFG["region_grid"][0] * _BELIEF_CFG["region_grid"][1])
...
class TestBeliefResetState(unittest.TestCase):
    ...
    def test_reset_belief_enc_all_zero(self):
        ...
        belief_enc = obs[BASE_OBS_DIM: BASE_OBS_DIM + 2 * _N_SLOTS]
        np.testing.assert_array_equal(
            belief_enc, np.zeros(2 * _N_SLOTS, dtype=np.float32)
        )

    def test_reset_staleness_all_ones(self):
        env = _make_belief_env()
        obs, _ = env.reset()
        staleness = obs[BASE_OBS_DIM + 2 * _N_SLOTS:]
        np.testing.assert_array_almost_equal(
            staleness, np.ones(_N_SLOTS, dtype=np.float32)
        )
...
class TestBeliefStepAppendsObs(unittest.TestCase):
    ...
    def test_step_staleness_drops_after_visible(self):
        ...
        staleness = obs_after[BASE_OBS_DIM + 2 * _N_SLOTS:]
